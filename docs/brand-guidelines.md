...
the pixel-art lakehouse scene with an interactive, mask-clipped water ripple
canvas. Ripple colors are drawn from the Blue ramp. The water mask
(`public/assets/hero-lakehouse-water-mask.png`) is generated by
[`scripts/make-hero-water-mask.py`](../scripts/make-hero-water-mask.py).
